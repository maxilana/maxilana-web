...
const transform = ({ blur, height, width, quality, format = 'jpg' }: TransformOptions) => {
  ...
  if (Number.isInteger(height) && Number.isInteger(width)) {
    sharpObj.resize(width, height, { fit: sharp.fit.cover, position: sharp.strategy.attention });

    // Only width set...
  } else if (isNumeric(width)) {
    sharpObj.resize(width, undefined, { fit: sharp.fit.cover, position: sharp.strategy.attention });

    // Only height set...
  } else if (isNumeric(height)) {
    sharpObj.resize(null, height, { fit: sharp.fit.cover, position: sharp.strategy.attention });
  }
